...
public static class ChickenFeatherVisualFactory
{
    const string Root = "World/ChickenFeather/Meshy_AI_Ivory_Feather_0714165218_texture_fbx/";
    const string ModelPath = Root + "Meshy_AI_Ivory_Feather_0714165218_texture";
    const string AlbedoPath = Root + "Meshy_AI_Ivory_Feather_0714165218_texture";
    const string NormalPath = Root + "Meshy_AI_Ivory_Feather_0714165218_texture_normal";
    const string MetallicPath = Root + "Meshy_AI_Ivory_Feather_0714165218_texture_metallic";

    static Material runtimeMaterial;

    public static bool IsFeather(string itemName)
    {
        return !string.IsNullOrWhiteSpace(itemName) &&
               string.Equals(itemName.Trim(), FeatherItemRegistry.ItemName, System.StringComparison.OrdinalIgnoreCase);
    }

    public static GameObject Spawn(Vector3 position, Item source, LayerMask groundMask, float collectRadius, Vector3 impulse)
    {
        GameObject drop = new GameObject("PenaGalinhaDrop");
        drop.transform.SetPositionAndRotation(position, Quaternion.Euler(0f, Random.Range(0f, 360f), 0f));
        CopyItemData(drop, source ?? FeatherItemRegistry.GetOrCreate());
        ...
        collider.size = new Vector3(0.54f, 0.22f, 0.3f);
        ...
        pickup.hoverHeight = 0.5f;
        pickup.bobHeight = 0.08f;
        pickup.rotationSpeed = 45f;
        return drop;
    }
    ...
    public static Sprite CreateThumbnailSprite(int textureSize = 96)
    {
        ...
        Vector3 origin = new Vector3(0f, -9900f, 0f);
        ...
        try
        {
            root = new GameObject("ChickenFeatherThumbnailRoot");
            ...
            Vector3 direction = new Vector3(0.8f, 1.2f, -2.2f).normalized;
            cameraObject = new GameObject("ChickenFeatherThumbnailCamera");
            ...
            lightObject = new GameObject("ChickenFeatherThumbnailLight");
            ...
            texture.name = "PenaGalinhaModelThumbnailTexture";
            Sprite sprite = Sprite.Create(texture, new Rect(0f, 0f, size, size), new Vector2(0.5f, 0.5f), size);
            sprite.name = "PenaGalinhaModelThumbnailSprite";
            return sprite;
        }
        finally
        {
    ...
    static void BuildVisual(Transform parent, float targetSize, Vector3 targetCenter)
    {
        ...
        GameObject visualRoot = new GameObject("ChickenFeatherVisual");
        visualRoot.transform.SetParent(parent, false);
        GameObject model = Object.Instantiate(prefab, visualRoot.transform);
        model.name = "MeshyChickenFeatherModel";
        StripGameplayComponents(model);
        ApplyMaterial(model);
        Normalize(visualRoot.transform, targetSize, targetCenter);
    }
    ...
    static Material GetMaterial()
    {
        ...
        runtimeMaterial = new Material(shader) { name = "ChickenFeather_RuntimeMaterial" };
    ...
    static void CopyItemData(GameObject target, Item source)
    {
        ...
        item.icon = FeatherItemRegistry.GetSprite();
