...
public static class BoarLeatherVisualFactory
{
    const string Root = "World/BoarLeather/Meshy_AI_Brown_Leather_Hide_0714183425_texture_fbx/";
    const string ModelPath = Root + "Meshy_AI_Brown_Leather_Hide_0714183425_texture";
    const string AlbedoPath = Root + "Meshy_AI_Brown_Leather_Hide_0714183425_texture";
    const string NormalPath = Root + "Meshy_AI_Brown_Leather_Hide_0714183425_texture_normal";
    const string MetallicPath = Root + "Meshy_AI_Brown_Leather_Hide_0714183425_texture_metallic";

    static Material runtimeMaterial;

    public static bool IsBoarLeather(string itemName)
    {
        return !string.IsNullOrWhiteSpace(itemName) &&
               string.Equals(itemName.Trim(), ThickLeatherItemRegistry.ItemName, System.StringComparison.OrdinalIgnoreCase);
    }

    public static GameObject Spawn(Vector3 position, Item source, LayerMask groundMask, float collectRadius, Vector3 impulse)
    {
        GameObject drop = new GameObject("CouroJavaliDrop");
        drop.transform.SetPositionAndRotation(position, Quaternion.Euler(0f, Random.Range(0f, 360f), 0f));
        CopyItemData(drop, source ?? ThickLeatherItemRegistry.GetOrCreate());
        BuildVisual(drop.transform, 0.68f, drop.transform.position);

        BoxCollider collider = drop.AddComponent<BoxCollider>();
        collider.center = new Vector3(0f, 0.08f, 0f);
        collider.size = new Vector3(0.62f, 0.2f, 0.48f);
        ...
        pickup.hoverHeight = 0.56f;
        pickup.bobHeight = 0.08f;
        pickup.rotationSpeed = 42f;
        return drop;
    }
    ...
    public static Sprite CreateThumbnailSprite(int textureSize = 96)
    {
        ...
        Vector3 origin = new Vector3(0f, -9950f, 0f);
        ...
        try
        {
            root = new GameObject("BoarLeatherThumbnailRoot");
            ...
            Vector3 direction = new Vector3(0.9f, 1.2f, -2.1f).normalized;
            cameraObject = new GameObject("BoarLeatherThumbnailCamera");
            ...
            lightObject = new GameObject("BoarLeatherThumbnailLight");
            ...
            texture.name = "CouroJavaliModelThumbnailTexture";
            Sprite result = Sprite.Create(texture, new Rect(0f, 0f, size, size), new Vector2(0.5f, 0.5f), size);
            result.name = "CouroJavaliModelThumbnailSprite";
            return result;
        }
        finally
        {
    ...
    static void BuildVisual(Transform parent, float targetSize, Vector3 targetCenter)
    {
        ...
        if (prefab == null)
            return;
        GameObject root = new GameObject("BoarLeatherVisual");
        root.transform.SetParent(parent, false);
        GameObject model = Object.Instantiate(prefab, root.transform);
        model.name = "MeshyBoarLeatherModel";
    ...
    static Material GetMaterial()
    {
        ...
        runtimeMaterial = new Material(shader) { name = "BoarLeather_RuntimeMaterial" };
    ...
    static void CopyItemData(GameObject target, Item source)
    {
        ...
        item.icon = ThickLeatherItemRegistry.GetSprite();
