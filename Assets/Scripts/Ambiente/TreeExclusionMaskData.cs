...
[CreateAssetMenu(fileName = "TreeExclusionMaskData", menuName = "World/Tree Exclusion Mask Data")]
public class TreeExclusionMaskData : ScriptableObject
{
    ...
    [Range(0f, 1f)] public float exclusionThreshold = 0.6f;
    ...
    public float SampleMask01(Vector2 worldPoint)
    {
        ...
        if (kernel == 0 || smoothSampleRadius <= 0f)
            return exclusionMask.GetPixelBilinear(uv.x, uv.y).grayscale;
        ...
        for (int y = -kernel; y <= kernel; y++)
        {
            for (int x = -kernel; x <= kernel; x++)
            {
                ...
                accum += exclusionMask.GetPixelBilinear(uv.x + offset.x, uv.y + offset.y).grayscale * weight;
                weightSum += weight;
            }
        }
    ...
    public bool IsTreeBlocked(Vector2 worldPoint)
    {
        return SampleMask01(worldPoint) >= exclusionThreshold;
    }
